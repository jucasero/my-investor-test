...
export const FundSellModal = ({ isOpen, onClose, portfolioItem }: FundSellModalProps) => {
  const { register, handleSubmit, watch, formState: { errors, isValid }, reset } = useForm<SellFormValues>({
    mode: 'onChange',
  });
  
  const { mutate: sellFund, isPending } = useSellFund();
  const quantity = watch('quantity');
  const unitPrice = portfolioItem ? portfolioItem.totalValue / portfolioItem.quantity : 0;
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} className={styles.container}>
        ...
        <div className={styles.field}>
            ...
            <span className={styles.value}>{formatCurrency(unitPrice, 'EUR')}</span>
        </div>

        <div className={styles.field}>
            <label htmlFor="quantity" className={styles.label}>Cantidad a vender</label>
            <input
                id="quantity"
                type="number"
                ...
                {...register('quantity', {
                    ...
                    validate: {
                        integer: (value) => Number.isInteger(Number(value)) || 'Solo números enteros',
                        maxOwn: (value) => Number(value) <= portfolioItem.quantity || `No puedes vender más de ${portfolioItem.quantity}`
                    }
                })}
            />
        ...
        <div className={styles.field}>
            ...
            <span className={styles.value}>{formatCurrency(estimatedTotal, 'EUR')}</span>
        </div>
