...
export const FundBuyModal = ({ isOpen, onClose, fund }: FundBuyModalProps) => {
  const { register, handleSubmit, watch, formState: { errors, isValid }, reset } = useForm<BuyFormValues>({
    mode: 'onChange',
  });
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} className={styles.container}>
        ...
        <div className={styles.field}>
            <label htmlFor="quantity" className={styles.label}>Cantidad a comprar</label>
            <input
                id="quantity"
                type="number"
                ...
                {...register('quantity', {
                    ...
                    validate: {
                        ...
                        maxTotal: (value) => (Number(value) * fund.value) <= 10000 || 'La compra no puede superar los 10.000,00 €'
                    }
                })}
            />
        ...
        <div className={styles.field}>
            ...
            <span className={styles.value}>{formatCurrency(total, fund.currency)}</span>
        </div>
