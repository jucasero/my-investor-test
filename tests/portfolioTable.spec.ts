...
test.describe("Portfolio table tests", () => {
  ...
  test("should view the details of a portfolio item", async ({ page }) => {
    const firstRowActions = page.locator(
      "tbody tr:nth-child(1) td:nth-child(4) button[aria-label='Actions']",
    );
  ...
  test("should sell a fund and validate inputs", async ({ page }) => {
    const availableQuantityText = await page
    ...
    await expect(
      page.getByText(`No puedes vender más de ${availableQuantity}`),
    ).toBeVisible();
    await expect(sellButton).toBeDisabled();
