...
test.describe('Funds table tests', () => {
  ...
  test('should view the details of a fund', async ({ page }) => {
    const firstRowActions = page.locator("tbody tr:nth-child(1) td:nth-child(9) div:nth-child(1) div:nth-child(1) button:nth-child(1) svg");
  ...
  test('should buy a fund and validate inputs', async ({ page }) => {
    ...
    await expect(page.getByText('La compra no puede superar los 10.000,00 €')).toBeVisible();
    await expect(buyButton).toBeDisabled();

    await quantityInput.fill('1');
    await expect(page.getByText('La cantidad debe ser mayor a 0')).not.toBeVisible();
    await expect(page.getByText('La compra no puede superar los 10.000,00 €')).not.toBeVisible();
